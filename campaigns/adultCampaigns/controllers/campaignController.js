...
const Adult_Campaigns = require('../models/Campaign');
...
exports.createCampaign = async (req, res) => {
  try {
    ...
    // Create new campaign
    const campaign = new Adult_Campaigns({
      campaignName,
...
exports.getAllCampaigns = async (req, res) => {
  try {
    ...
    const campaigns = await Adult_Campaigns.find(query)
      .sort(sort)
      .skip(skip)
      .limit(parseInt(limit));

    const total = await Adult_Campaigns.countDocuments(query);
...
exports.getCampaignById = async (req, res) => {
  try {
    ...
    const campaign = await Adult_Campaigns.findById(id);
...
exports.deleteCampaign = async (req, res) => {
  try {
    ...
    const campaign = await Adult_Campaigns.findByIdAndDelete(id);
...
exports.getCampaignStats = async (req, res) => {
  try {
    const stats = await Adult_Campaigns.aggregate([
      {
        $group: {
          _id: '$status',
          count: { $sum: 1 },
    ...
    const totalCampaigns = await Adult_Campaigns.countDocuments();
    const totalCost = await Adult_Campaigns.aggregate([
      {
        $group: {
          _id: null,
          total: { $sum: '$totalCost' }
...
exports.duplicateCampaign = async (req, res) => {
  try {
    ...
    const originalCampaign = await Adult_Campaigns.findById(id);
    ...
    const duplicatedCampaign = new Adult_Campaigns({
...
exports.bulkDeleteCampaigns = async (req, res) => {
  try {
    ...
    const result = await Adult_Campaigns.deleteMany({ _id: { $in: ids } });
