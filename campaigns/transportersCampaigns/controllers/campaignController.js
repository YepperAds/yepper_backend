...
const Transporter_Campaigns = require('../models/Campaign');
...
exports.createCampaign = async (req, res) => {
  try {
    ...
    // Create new campaign
    const campaign = new Transporter_Campaigns({
      campaignName,
...
exports.getAllCampaigns = async (req, res) => {
  try {
    ...
    const campaigns = await Transporter_Campaigns.find(query)
      .sort(sort)
      .skip(skip)
      .limit(parseInt(limit));

    const total = await Transporter_Campaigns.countDocuments(query);
...
exports.getCampaignById = async (req, res) => {
  try {
    ...
    const campaign = await Transporter_Campaigns.findById(id);
...
exports.deleteCampaign = async (req, res) => {
  try {
    ...
    const campaign = await Transporter_Campaigns.findByIdAndDelete(id);
...
exports.getCampaignStats = async (req, res) => {
  try {
    const stats = await Transporter_Campaigns.aggregate([
      {
        $group: {
          _id: '$status',
          count: { $sum: 1 },
    ...
    const totalCampaigns = await Transporter_Campaigns.countDocuments();
    const totalCost = await Transporter_Campaigns.aggregate([
      {
        $group: {
          _id: null,
          total: { $sum: '$totalCost' }
...
exports.duplicateCampaign = async (req, res) => {
  try {
    ...
    const originalCampaign = await Transporter_Campaigns.findById(id);
    ...
    const duplicatedCampaign = new Transporter_Campaigns({
...
exports.bulkDeleteCampaigns = async (req, res) => {
  try {
    ...
    const result = await Transporter_Campaigns.deleteMany({ _id: { $in: ids } });
