...
const CarOwners_Campaigns = require('../models/Campaign');
...
exports.createCampaign = async (req, res) => {
  try {
    ...
    // Create new campaign
    const campaign = new CarOwners_Campaigns({
      campaignName,
...
exports.getAllCampaigns = async (req, res) => {
  try {
    ...
    const campaigns = await CarOwners_Campaigns.find(query)
      .sort(sort)
      .skip(skip)
      .limit(parseInt(limit));

    const total = await CarOwners_Campaigns.countDocuments(query);
...
exports.getCampaignById = async (req, res) => {
  try {
    ...
    const campaign = await CarOwners_Campaigns.findById(id);
...
exports.deleteCampaign = async (req, res) => {
  try {
    ...
    const campaign = await CarOwners_Campaigns.findByIdAndDelete(id);
...
exports.getCampaignStats = async (req, res) => {
  try {
    const stats = await CarOwners_Campaigns.aggregate([
      {
        $group: {
          _id: '$status',
          count: { $sum: 1 },
    ...
    const totalCampaigns = await CarOwners_Campaigns.countDocuments();
    const totalCost = await CarOwners_Campaigns.aggregate([
      {
        $group: {
          _id: null,
          total: { $sum: '$totalCost' }
...
exports.duplicateCampaign = async (req, res) => {
  try {
    ...
    const originalCampaign = await CarOwners_Campaigns.findById(id);
    ...
    const duplicatedCampaign = new CarOwners_Campaigns({
...
exports.bulkDeleteCampaigns = async (req, res) => {
  try {
    ...
    const result = await CarOwners_Campaigns.deleteMany({ _id: { $in: ids } });
