...
const Parents_Campaigns = require('../models/Campaign');
...
exports.createCampaign = async (req, res) => {
  try {
    ...
    // Create new campaign
    const campaign = new Parents_Campaigns({
      campaignName,
...
exports.getAllCampaigns = async (req, res) => {
  try {
    ...
    const campaigns = await Parents_Campaigns.find(query)
      .sort(sort)
      .skip(skip)
      .limit(parseInt(limit));

    const total = await Parents_Campaigns.countDocuments(query);
...
exports.getCampaignById = async (req, res) => {
  try {
    ...
    const campaign = await Parents_Campaigns.findById(id);
...
exports.deleteCampaign = async (req, res) => {
  try {
    ...
    const campaign = await Parents_Campaigns.findByIdAndDelete(id);
...
exports.getCampaignStats = async (req, res) => {
  try {
    const stats = await Parents_Campaigns.aggregate([
      {
        $group: {
          _id: '$status',
          count: { $sum: 1 },
    ...
    const totalCampaigns = await Parents_Campaigns.countDocuments();
    const totalCost = await Parents_Campaigns.aggregate([
      {
        $group: {
          _id: null,
          total: { $sum: '$totalCost' }
...
exports.duplicateCampaign = async (req, res) => {
  try {
    ...
    const originalCampaign = await Parents_Campaigns.findById(id);
    ...
    const duplicatedCampaign = new Parents_Campaigns({
...
exports.bulkDeleteCampaigns = async (req, res) => {
  try {
    ...
    const result = await Parents_Campaigns.deleteMany({ _id: { $in: ids } });
