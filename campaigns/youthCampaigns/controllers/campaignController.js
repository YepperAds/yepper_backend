...
const Youth_Campaigns = require('../models/Campaign');
...
exports.createCampaign = async (req, res) => {
  try {
    ...
    // Create new campaign
    const campaign = new Youth_Campaigns({
      campaignName,
...
exports.getAllCampaigns = async (req, res) => {
  try {
    ...
    const campaigns = await Youth_Campaigns.find(query)
      .sort(sort)
      .skip(skip)
      .limit(parseInt(limit));

    const total = await Youth_Campaigns.countDocuments(query);
...
exports.getCampaignById = async (req, res) => {
  try {
    ...
    const campaign = await Youth_Campaigns.findById(id);
...
exports.deleteCampaign = async (req, res) => {
  try {
    ...
    const campaign = await Youth_Campaigns.findByIdAndDelete(id);
...
exports.getCampaignStats = async (req, res) => {
  try {
    const stats = await Youth_Campaigns.aggregate([
      {
        $group: {
          _id: '$status',
          count: { $sum: 1 },
    ...
    const totalCampaigns = await Youth_Campaigns.countDocuments();
    const totalCost = await Youth_Campaigns.aggregate([
      {
        $group: {
          _id: null,
          total: { $sum: '$totalCost' }
...
exports.duplicateCampaign = async (req, res) => {
  try {
    ...
    const originalCampaign = await Youth_Campaigns.findById(id);
    ...
    const duplicatedCampaign = new Youth_Campaigns({
...
exports.bulkDeleteCampaigns = async (req, res) => {
  try {
    ...
    const result = await Youth_Campaigns.deleteMany({ _id: { $in: ids } });
