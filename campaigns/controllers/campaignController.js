...
exports.getCampaignStats = async (req, res) => {
  try {
    const stats = await Campaigns.aggregate([
      {
        $group: {
          _id: '$status',
          count: { $sum: 1 },
    ...
    const totalCost = await Campaigns.aggregate([
      {
        $group: {
          _id: null,
          total: { $sum: '$totalCost' }
...
exports.duplicateCampaign = async (req, res) => {
  try {
    ...
    const originalCampaign = await Campaigns.findById(id);
    ...
    const duplicatedCampaign = new Campaigns({
...
exports.bulkDeleteCampaigns = async (req, res) => {
  try {
    ...
    const result = await Campaigns.deleteMany({ _id: { $in: ids } });
